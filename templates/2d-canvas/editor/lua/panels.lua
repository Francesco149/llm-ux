...
function panels.begin(id, rect, title, opts)
  ...
  if title then
    local dl = ig.get_window_draw_list()
    local y0 = rect.y
    dl_add_rect_filled(dl, rect.x, y0, rect.x + rect.w, y0 + 26, 0.11, 0.115, 0.14, 1)
    dl_add_rect_filled(dl, rect.x, y0 + 25, rect.x + rect.w, y0 + 26, 0.18, 0.19, 0.23, 1)
...
function panels.toolbar(rect)
  ...
  if doc.loaded then
    ...
    if ig.is_item_hovered() then
      ig.begin_tooltip(); ig.text("Undo (Ctrl+Z)"); ig.end_tooltip()
    end
    ig.same_line()
    if ig.button("Redo") then undo.do_redo() end
    if ig.is_item_hovered() then
      ig.begin_tooltip(); ig.text("Redo (Ctrl+Y)"); ig.end_tooltip()
    end
    ig.same_line()
    if ig.button("Import (Ctrl+U)") then import.open_dialog() end
    if ig.is_item_hovered() then
      ig.begin_tooltip(); ig.text("Import Image Asset (Ctrl+U)"); ig.end_tooltip()
    end
    ig.same_line()
    if ig.button("Export (Ctrl+E)") then export.all() end
    if ig.is_item_hovered() then
      ig.begin_tooltip(); ig.text("Export All Active Outputs (Ctrl+E)"); ig.end_tooltip()
    end
    ig.same_line()
