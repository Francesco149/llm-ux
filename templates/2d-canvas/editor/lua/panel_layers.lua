...
local function row(l, depth, in_flight)
  ...
  -- type badge measurement (for dynamic name width calculation)
  local badge = doc.type_names[l.type]
  local tw_ = ig.calc_text_size(badge)
  local show_badge = (w > 200)
  local name_w = math.max(30, w - 88 - (show_badge and (tw_ + 12) or 0))

  -- name selectable (width constrained so it never overlaps the badge)
  ig.set_next_item_width(name_w)
  local hit = ig.selectable(l.name .. "##" .. l.id, selected, 0, name_w, row_h - 6)
  if hit then panels.set_selected(l.id) end

  -- type badge on the right (hidden on very narrow panels to prioritize name)
  if show_badge then
